...
|---|---|---|---|
| 1 | Self-play Elo vs previous version, SPRT `elo0=0, elo1=5, α=β=0.05` | pass before merge | **PARTIAL** — bot4 vs bot1fix 30 games, LLR +0.31 of +2.94. Machinery exists (`tools/h2h.py`, now with `--workers`); needs **~300 games at `elo1=5`, not 130** — see §6. ~1 h on the PC at 6-way. Finish it. |
...
- **bot4 has not passed SPRT.** 30 of ~300 games (§6 — the old 130 estimate was wrong). This is the single thing
...
0. **Finish bot4's SPRT first (~100 more games). Runs on the PC, ~40 min.** bot4
   ...
   and grind is exactly what the PC is for. `--workers 6` is implemented and
   tested, so this is one command: `BUILD=bot4_ordering VS=bot1_baseline make
   sprt`. Budget ~300 games at `elo1=5`, or ~90 at `elo1=20`.
